export function stringFormat(str, arrayArgs) {
  let args;
  let offset;
...
export const AllIsDoneMultiplexer = (function () {
...
export function toDict(asArray) {
...
export function addRange(toAdd, toAddTo) {
  toAddTo = toAddTo || [];

  toAdd.forEach(function (addField) {
    let addName = addField.name;
    let index = toAddTo.findIndex(function (existingField) {
